...
P.render=function(){
 ...
 const bg=A.images.meadow; if(bg)c.drawImage(bg,-22-Math.sin(this.camX/5500)*18,-58,1004,670);
 const sx=this.shake>0?Math.sin(t*91)*this.shake*.22:0;c.translate(sx,0);
 const tile=A.images.platform;
 if(tile){const start=-((this.camX)%240);for(let x=start-240;x<C.W+240;x+=240)c.drawImage(tile,tile.width*.15,0,tile.width*.7,tile.height,x,C.GROUND_Y,242,100)}
 ...
 const key=h.kind==='rock'?'mushroom':h.kind==='mob'?'chimera':'weed';
 ...
 for(let i=2;i>=0;i--){const x=p.px-this.lead-i*46;A.draw(c,'chimera',x-40,C.GROUND_Y-70-Math.abs(Math.sin(t*9+i))*4,80,70)}
...
P.updateHud=function(){
 ...
 set('hud-flight',this.p.flyReason==='item'?'应援飞行 '+Math.max(0,this.p.flyTimer).toFixed(1)+'s':'');
